...
import onHoldIcon from "@/app/assets/onHold.svg";
import { EnrollmentResponse } from "@/types/enrollmentApiTypes";
import { Button } from "@/components/ui/button";

export default function OnHold({ enrollmentData }: { enrollmentData: EnrollmentResponse }) {
  ...
  return (
    <div className="grid grid-col-1 lg:grid-cols-3 container mx-auto p-6 lg:p-8 gap-6">
      <div className="col-span-2 w-full h-fit flex flex-col gap-6">
        <div className="w-full h-fit border border-[#EBEBEB] bg-[#F7F7F7] rounded-[8px] shadow-none p-4 flex items-center gap-3 relative">
          <Image src={onHoldIcon} alt="On Hold Icon" width={40} height={40} />
          <div className="flex flex-col">
            <h2 className="text-black-tint-70 font-bold text-xl">
              {t("On Hold")}
            </h2>
            <p className="text-xs sm:text-sm md:text-base text-black-tint-90 font-normal">
              {t(
                "Your application is on hold at the moment, please pay the registration fee to continue"
              )}
            </p>
          </div>
          <div
            className={`absolute bg-black-tint-70 w-2 h-full ${
              locale === "en" ? "left-0 " : "right-0"
            } rounded-s-[8px]`}
          ></div>
        </div>

        <div className="border border-black-tint-10 rounded-[8px] shadow-none py-4 px-5 h-20 bg-p-tints-tint-3 flex justify-between items-center">
          <h3 className="text-main-primary font-bold text-lg flex gap-[2px] items-center">
            {t("Registration Fee:")} 100 <SaudiRiyal className="w-4 h-4" />
          </h3>
          <div className="flex items-center gap-4">
            <p className="text-black-tint-60 font-normal text-sm">
              {t("Registration Fee are not refundable")}
            </p>
          ...
          {previousApplications.map((application) => (
            ...
            >
              <div className="flex-1 min-w-0 flex gap-2 items-center">
                <div className="bg-black-tint-5 w-12 h-12 flex justify-center items-center rounded-full">
                  <CircleAlert className="text-black-tint-70" />
                </div>
                <div>
